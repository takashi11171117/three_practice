...
const GLSL: FC = () => {
    const [material, setMaterial] = useState(new ShaderMaterial( {
        uniforms: {
    ...
    const move = (evt) => {
        material.uniforms.u_mouse.value.x = (evt.touches) ? evt.touches[0].clientX : evt.clientX;
        material.uniforms.u_mouse.value.y = (evt.touches) ? evt.touches[0].clientY : evt.clientY;
    ...
    // animation
    const animate = (params: { composer: EffectComposer; clock: Clock; plane: Mesh<PlaneGeometry, ShaderMaterial> }) => {
